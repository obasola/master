...
describe('services', function(){
  ...
  describe('Publisher', function(){
    ...
    it('create throws exception : id not defined', function() {
    	try{
    		Publisher.create({id:null,name:'publisher'});
    		$httpBackend.flush();
    	} catch(errors) {
    ...
    it('create', function() {
    	...
    	Publisher.create({id:'1',name:'publisher'}).then(function(response) {
        	expect(response.status).toBe(200);
    		expect(response.data).toBe("test");
    	});
    	$httpBackend.flush();
    ...
    it('update throws exception : id not defined', function() {
    	try{
    		Publisher.update({id:null,name:'publisher'});
    		$httpBackend.flush();
    	} catch(errors) {
    		expect(errors[0]).toBe('publisher.id.not.defined');
    	}
    ...
    it('update', function() {
    	...
    	Publisher.update({id:'1',name:'publisher'}).then(function(response) {
        	expect(response.status).toBe(200);
    		expect(response.data).toBe("test");
    	});
    	$httpBackend.flush();
